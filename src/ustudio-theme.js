...
if (config.studio_uid === "" || config.access_token === "") {
  console.error(
    "ERROR Please fill out the studio uid and access token in config.json".error
  );
  return;
}
...
function ustudio_theme(options) {
  var opts = require("optimist")
      .usage(
        "Usage: $0 <method> -t [theme name] -t [destination uid] --debug --test"
      )
      .demand(["$0"])
      .alias("t", "theme")
  ...
  function valid_theme() {
    if (!theme) {
      console.error(
        "ERROR a theme must be provided using the -t [theme] option".error
      );
      return false;
    }
    return true;
  ...
  function valid_destination() {
    if (!destination) {
      console.error(
        "ERROR a destination must be provided using the -d [destination uid] option"
          .error
      );
      return false;
    }
  ...
  function valid_module() {
    if (!module) {
      console.error(
        "ERROR a module must be provided using the -m [module] option".error
      );
      return false;
    }
    return true;
  ...
  switch (method) {
    ...
    case "list":
      if (DEBUG) {
        console.log("CALL list_themes".debug);
      }
    ...
    case "enable-module":
      if (valid_module() && valid_destination()) {
        ustudio_theme.destination_enable_player_module(
          destination,
          module,
          disable
        );
...
var _tpl = {
  ...
  msg_upload_status_success: _.template(
    "SUCCESS {{type}} {{name}} (uid: {{uid}}) uploaded to uStudio ({{status}})"
  ),
...
function ustudio_theme_settings(type, name) {
  ...
  return {
    dir: {
      path: ustudio_dir_path,
      EXISTS: ustudio_dir_exists,
      files: files,
      js: js_files,
    },
    json: {
      file: ustudio_json,
      path: ustudio_json_path,
      EXISTS: ustudio_json_exists,
    },
    zip: {
      file: ustudio_zip_file,
      path: ustudio_zip_path,
      EXISTS: ustudio_zip_exists,
    },
  };
}
...
ustudio_theme.create = function (type, name, endpoint, callback) {
  ...
  if (config.dir.EXISTS) {
    ...
    if (DEBUG) {
      console.log("    files: ".debug + util.inspect(files, { colors: true }));
    }
    ...
    if (DEBUG) {
      console.log("    No " + config.json.file + " found".debug);
    }
  }
...
ustudio_theme.update = function (type, name, endpoint, getData, callback) {
  ...
  if (!config.json.EXISTS) {
    console.error(
      "ERROR " + config.json.file + " not found ".error + config.json.path
    );
    return;
  }
  var config_json = JSON.parse(fs.readFileSync(config.json.path)),
  ...
  request(
    ...
    function (response_str) {
      try {
        ...
        if (DEBUG) {
          console.log(
            "RESPONSE: ".debug + util.inspect(response, { colors: true })
          );
        }
        if (response.error) {
...
ustudio_theme.upload = function (type, name, callback) {
  ...
  if (!config.json.EXISTS) {
    console.error(
      ("ERROR " + config.json.file + " file not found: ").error +
        config.json.path
    );
    return;
  }
  if (type !== "module" && !config.zip.EXISTS) {
    console.error(
      ("ERROR " + config.zip.file + " not found: ").error + config.zip.path
    );
    return;
  }
  ...
  if (DEBUG) {
    console.log(
      _tpl.msg_upload_status({ type: type, name: name }).debug + config.zip.path
    );
  }
  ...
  request(
    ...
    function (response, statusCode) {
      ...
      if (statusCode >= 200 && statusCode < 300) {
        console.log(
          _tpl.msg_upload_status_success({
            type: type,
            name: name,
            uid: config_json.uid,
            status: statusCode,
        ...
        console.error(
          ("ERROR " + statusCode + " unable to upload " + type).error
        );
      }
      if (callback) {
...
ustudio_theme.upload_theme = function (theme_name, done) {
  ...
  if (DEBUG) {
    console.log(("UPLOAD theme " + theme_name + " ").debug + theme_zip_path);
  }
  console.log("Uploading please wait...".info);
  var form,
    formHeaders = null;
  form = new formData();
  formHeaders = Object.assign(form.getHeaders(), {
    "X-Auth-Token": config.access_token,
  });
  form.append("package", fs.createReadStream(theme_zip_path));
  axios
    .post(path, form, {
      headers: formHeaders,
    })
...
ustudio_theme.create_theme = function (theme_name) {
  ...
  if (DEBUG) {
    console.log(
      "  Checking for existing theme.json in ".debug + theme_dir_path
    );
  }
  if (theme_dir_exists) {
    ...
    if (DEBUG) {
      console.log("    No theme.json found".debug);
    }
  }
...
ustudio_theme.destination_set_theme = function (
  destination_uid,
  theme_name,
  disable
) {
  ustudio_theme.update(
    "theme",
    theme_name,
    uStudioAPIEndpoints.destination(destination_uid),
    function (theme_json) {
      return {
...
ustudio_theme.create_player_module = function (player_module_name, callback) {
  ustudio_theme.create(
    "module",
    player_module_name,
    uStudioAPIEndpoints.modules(),
    callback
  );
};
...
ustudio_theme.delete_player_module = function (player_module_name) {
  ...
  var config_json = JSON.parse(fs.readFileSync(config.json.path)),
    module_uid = config_json.uid;
  ustudio_theme.delete(
    "module",
    player_module_name,
    uStudioAPIEndpoints.module(module_uid)
  );
};

//### Enable Module on Destination
ustudio_theme.destination_enable_player_module = function (
  destination_uid,
  player_module_name,
  disable
) {
  ustudio_theme.update(
    "module",
    player_module_name,
    uStudioAPIEndpoints.destination_player_modules(destination_uid),
...
_.each(_.keys(ustudio_theme), function (fn) {
  if (typeof ustudio_theme[fn] === "function") {
    ustudio_theme[fn] = _.wrap(ustudio_theme[fn], function (func) {
      ...
      if (DEBUG) {
        console.log(
          ("CALL " + fn + ": ").debug + util.inspect(args, { colors: true })
        );
      }
  ...
  if (!TEST) {
    if (options.form) {
      ...
      var req = https.request(
        post_options,
        _.partial(response_callback, callback, null)
      );
